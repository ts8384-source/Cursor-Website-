...
**Honesty:** this page names SOTA *ideas* from `GET /api/papers/db`. We do **not** run those pipelines. If a catalog file is abstract-only, the row says so. Do not treat the literature table as a feature list.

## What a useful record now includes

`GET /api/meta/:id` should be enough to navigate without guessing:

| Field | Why it is there |
| --- | --- |
| `id` / `type` / `title` | Stable key |
| `href` | Site path (`/site/<slug>` or `/site/papers#<id>`) |
| `parent` / `children` | Wiki nest |
| `nest` | `encyclopedia` \| `sandbox` \| `hidden` \| `corpus` |
| `sandbox` / `sandboxLane` | Lab vs main |
| `flags.hidden` / `flags.flagged` / `flags.highlight` | Retired, come-back, Start/Queue/Lab |
| `graphDegree` | Count of allowed wiki-graph edges |
| `citationCount` / `relatedCount` | Cheap size of those arrays |
| `summary.short` / `summary.long` | Chrome vs agent fetch |

Filters: `?nest=sandbox`, `?parent=page:sandbox`, `?flagged=1`, plus the old `q` / `type` / `tag` / `citation` / `related`.

Worked example: `GET /api/meta/page:future` → `nest: sandbox`, `parent: page:sandbox`, `href: /site/future`. `GET /api/meta/page:hybrid-rag` → `nest: encyclopedia`. `GET /api/meta?type=paper` rows use `nest: corpus`.
