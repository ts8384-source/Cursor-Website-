...
Gists here are future reference. They must not enter `/api/memory` decay [@agent-workflow-memory-2024]. Paper Plain: gist beside source, not a replacement [@paper-plain-august-2023].

## How / Tools

`GET /api/papers/db`, `POST /api/papers/db/rebuild`, catalog on [Papers](/site/papers). Meta: `GET /api/meta?nest=corpus`.

## Worked example

Open `living-papers-heer-2023` on the board below. The same id is `GET /api/meta/living-papers-heer-2023` with `href: /site/papers#living-papers-heer-2023`.
