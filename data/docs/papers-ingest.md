...
Tried via `POST /api/papers/fetch` during a lab optimization pass. See [Future work](/site/future). Client sockets used to die at 15s (`server.timeout`); the handler still wrote MD. HTTP timeouts are now 400s so a later call can wait for seed + rebuild.

| Work | Asked id | Result |
| --- | --- | --- |
| FadeMem | `2601.18642` / `arxiv-2601-18642` | **In DB** (already). Search hit `papers:arxiv-2601-18642-md`. |
| MemoryBank | `2305.10250` / `arxiv-2305-10250` | **In DB** (already). Search hit `papers:arxiv-2305-10250-md`. |
| Memory as Metabolism (Miteski) | `2604.12034` / `arxiv-2604-12034` | **In DB** (already). Search hit `papers:arxiv-2604-12034-md`. |
| MemoBrain | `2601.08079` | **Landed.** `arxiv-2601-08079`. Title: *MemoBrain: Executive Memory as an Agentic Brain for Reasoning*. Search hit `papers:arxiv-2601-08079-md`. |
| ClawVM (Rafique & Bindschaedler) | `2604.10352` | **Confirmed OA and landed.** `arxiv-2604-10352`. EuroMLSys 2026. Explore only — not on the implement queue. |
| CompactRAG | `2602.05728` | **Landed.** `arxiv-2602-05728`. Search hit `papers:arxiv-2602-05728-md`. |
| Learning to Share (LTS) | `2602.05965` | **Landed.** `arxiv-2602-05965`. Search hit `papers:arxiv-2602-05965-md`. |
| Context Rot (Chroma Research) | no arXiv; HTML `https://research.trychroma.com/context-rot` | **Not in DB.** Fetch refused: `not an OA PDF (use an arXiv id or a direct PDF URL)`. Blog/report HTML is not a paper. |
| SemaClaw | — | **Not in DB.** No OA id this turn. |

Atom `<title>` used to steal the feed title (`arXiv Query:…`). Parser now reads the `<entry>` title. The four new files were retitled from the OA records. Then `python -m rag.cli seed` + `rebuild --grounds papers` (2526 paper chunks). `GET /api/papers/db` picked them up when catalog length changed.
