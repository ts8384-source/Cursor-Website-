...
Expanded this pass: wiki-memory, Workbench nest, Lab bench, How the lab works, Metadata, Hybrid RAG, Papers, Memory, Agents, Knowledge graph, CONTROL, plus lab writeup `/site/sandbox-product-program`. Overview and Implement stay **intentionally** short chrome (map / queue). Do not turn those two into essays.

## Website Lab bench

New drafts default to `data/md/sandbox/` with `parent: page:sandbox` and optional `sandboxLane`. They nest under [Lab bench](/site/sandbox). Explain copy is [How the lab works](/site/sandbox-how-the-lab-works). Chat points at `/site/<slug>`. Promote only after **promote to main**.
