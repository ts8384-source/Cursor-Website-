...
Without `executedIn`, agents invent “this paper lives in file X.” Curated rows plus cite-or-fetch keep that honest [@living-papers-heer-2023].

## How / Tools

`GET /api/execution`, `POST /api/papers/db/rebuild`. Files: `backend/persist/paper-curated.ts`, `data/md/execution.md`.

## Worked example

Living Papers → `backend/persist/pages.ts` and `data/md/`. ViDoRAG stays **cited only** — no VLM row pretending we shipped it.
