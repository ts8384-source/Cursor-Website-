...
- **Karpathy wiki compiler.** [Wiki memory](/site/wiki-memory) compares the April 2026 cluster. We did not add a `wiki/` compile step or a two-process crew. Gists and the llm-wiki-memory-template wiki are still **not in the paper DB** (not OA PDFs).
- **Imported later hats.** [Agents](/site/agents) are thin isolated skills. Paper Lantern MCP / IntrAgent / Caesar / MGM evolution stay unbuilt. The flagged-schemes dump is retired. Workbench nest is removed. ClawVM stays unbuilt.
