...
A single fused ranker would mix a paper abstract with a parked scribble and a `@chunk`. HM-RAG and HetaRAG license **multiple stores**; they do not license one RRF soup [@hm-rag-2025] [@heta-rag-2025]. Cite-or-fetch then opens the real id.

## How (agent)

1. `GET /api/search?q=` — read `per_ground` and labeled hits.
2. Optional `GET /api/graph/neighborhood?id=` — labeled `graph` context, then open the page or paper.
3. `GET /api/meta/:id` — `summary.long`, `related`, `nest`, `graphDegree`.
4. Missing named OA → **not in DB** + `POST /api/papers/fetch`.

## Tools / MD paths

| Tool | Role |
| --- | --- |
| `GET /api/search` | Isolated grounds |
| `POST /api/ask` | Scribble ingest + cite envelope |
| `GET /api/meta` | Title/tag boost, not a sixth RRF library |
| `python -m rag.cli seed` / `rebuild` | Copy then index |

Code: `backend/retrieve/hybrid.ts`, `rag/`. Page: `data/md/hybrid-rag.md`.

## Worked example

Search `hybrid rag` should hit `page:hybrid-rag` as a labeled `meta` or `md` record and `code:loop.hybrid_search` on the code ground. Those ranks stay separate. The [knowledge graph](/site/knowledge-graph) may prepend neighbors; it does not fuse scores.
