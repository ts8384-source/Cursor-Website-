...
summaryShort: Permanent OA catalog
summaryLong: Disk catalog plus GET /api/papers/db. Child papers-ingest is the OA recipe. Papers never forget. Meta nest for papers is corpus.
questions:
...
Permanent store: `data/papers` (Markdown under 400 KB for seed; PDFs in `data/papers/pdf/`). The **must-need paper DB** is `GET /api/papers/db` — summaries, figures, math terms, execution rows. Forgetting never applies here. The handbook chapters in `refs/ui-rag` also land on the **papers** ground. This page does not paste retrieve hits. The list below the article is `GET /api/papers`. Search the corpus box (hybrid RAG) and filter the catalog — overview first, passages on demand. Use **Fetch paper** for an arXiv id or OA PDF URL (`POST /api/papers/fetch`). Full gists live on [Summaries](/site/summaries). `GET /api/meta?nest=corpus` lists the same paper ids with `href` `/site/papers#<id>` and `related` pages that cite them.

## Why

Cite-or-fetch needs a **named disk catalog**. Chat memory is the wrong store. Living Papers treats the MD extract as the local paper record [@living-papers-heer-2023]. Paper Plain / Semantic Reader chrome sits on this site, not inside the PDF viewer [@paper-plain-august-2023] [@semantic-reader-project-2023].

## How

1. Confirm the id on `GET /api/papers` / `GET /api/papers/db`.
2. If missing and OA: [Internet → local](/site/papers-ingest) or the form on this page.
3. Retrieve labeled `ground: papers` hits. Do not invent abstracts.
4. Cite `[@catalog-id]` on the page that needed the paper.

## Tools / MD paths

`GET /api/papers`, `GET /api/papers/:id`, `GET /api/papers/db`, `POST /api/papers/fetch`, `GET /api/meta?type=paper`. Files: `data/papers/`, `data/md/papers.md`, `data/md/papers-ingest.md`.

## Worked example

`living-papers-heer-2023` is in DB and cited on this page. A filter `GET /api/meta?citation=living-papers-heer-2023` returns pages (including this one) that list that id. ColPali is **named** as future diagram retrieve; do not claim we indexed page images [@colpali-2024].
