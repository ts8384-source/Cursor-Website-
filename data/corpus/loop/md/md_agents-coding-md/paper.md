...
Not in the default mash. Hire hook is **implemented**; evolution is not. **User starts the hire.** If stuck, ask the operator first — do not auto-arm. Never “production.” [MGM on-demand](/site/workbench-mgm). [Debug nest](/site/agents-coding-debug).

## Sandbox

Fork / lab first (`POST /api/sandbox/fork`). Work **in the lab** until he says send back. Human check (`propose`); no auto-merge. No Docker mandate. [Sandbox](/site/workbench-sandbox). [Generation nest](/site/agents-coding-generate). Queue: [To-implement](/site/implement).
