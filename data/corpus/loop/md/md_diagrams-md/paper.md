...
Temporary §8 graphs (`wiki-memory-compare`, `wiki-memory-grounding`) live on this board’s switcher and on [Wiki memory (temp)](/site/wiki-memory). The standing two-side compare (`workbench-compare`) lives on [Workbench](/site/workbench). They are think-through flows, not an implemented compiler. The three-mash index (`flagged-schemes-loop`) sits on [Agents](/site/agents). Those regions are hats, not iPad / local PC / website. The live wiki graph is [Knowledge graph](/site/knowledge-graph).
