...
export function SiteApp() {
  ...
  useEffect(() => {
    void fetch(apiUrl('/api/pages'))
      .then((r) => r.json())
      .then((data: { pages?: PageMeta[]; tree?: PageTreeNode[] }) => {
  ...
  return (
    <div className="site-shell">
      ...
      <header className="site-top">
        ...
        <p className="crumb">
          <Link className="wiki-hl wiki-hl-start crumb-hl" to="/site/overview">
            Overview <span className="wiki-badge wiki-badge-start">Start</span>
          </Link>
          {crumbs
        ...
        <main id="article" className="site-article" tabIndex={-1} onClick={onArticleClick}>
          {page?.highlight === 'start' ? (
            ...
            <p className="lab-banner" role="status">
              <span className="wiki-badge wiki-badge-lab">Lab</span>{' '}
              <strong>Olive petri.</strong> Explorations nest here until promote to main — not encyclopedia peers. Handbook 7:2 / 9:1: grouped and labeled, not color-only.
            </p>
          ...
          {slug === 'overview' ? <OverviewStructure tree={tree} slug={slug} /> : null}
