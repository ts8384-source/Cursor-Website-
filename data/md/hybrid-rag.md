...
- **seed** (`python -m rag.cli seed` or `POST /api/scripts` `{ "name": "seed" }`) copies drop folders into `data/corpus/loop/<ground>/`. Handbook Markdown is ingested into **papers**. `data/md` and `content/` go into **md**.
