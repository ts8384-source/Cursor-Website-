...
glossary:
  ...
  - term: To-implement
    def: Write/erase work queue (GET/POST /api/implement). Not the Overview page tree. Queue highlight, not a Workbench nest.
  - term: depth
  ...
  - term: website sandbox
    def: Lab bench at /site/sandbox (highlight lab). Agents write children under data/md/sandbox/. How-to copy is /docs/sandbox-how-the-lab-works (doc bin), not a lane. Git fork is POST /api/sandbox/fork. Trash bin is GET/POST /api/sandbox/trash (lab only, 14-day forget). Lanes idea | code | research.
  - term: boot wiki
    def: User project pages in data/md. Routes /site. New forker pages go here. Summary starts empty.
  - term: doc wiki
    def: Framework explain pages in data/docs. Routes /docs. One-folder delete. Missing folder → boot still runs.
  - term: condensed return
    def: Isolated subagent packet — stable ids plus a 1–3 sentence gist. GET/POST /api/condensed. Not a transcript.
citations:
  - swe-agent-2024
  - gorilla-2023
  - webarena-2024
...
This file is the **primary control surface** for humans and agents. It is Markdown on disk (`content/CONTROL.md`) and a page on the **doc** site (`/docs/control`) while `data/docs` exists. That is the Living Papers idea applied to operations: one source, machine-readable list via `GET /api/pages`, not a hidden wiki in chat [@living-papers-heer-2023].
...
## Strict sandbox output (no chat-only dumps)

**Every agent turn that does work writes a Lab-bench page.** Chat is a pointer.

1. Create or update `data/md/sandbox/<slug>.md` (`sandbox: true`, `parent: page:sandbox` or a lane parent, `sandboxLane: idea|code|research` except the how-to child).
2. Chat **only** names `/site/<slug>`. Do not paste the article into the transcript as if that were the wiki.
3. Lab bench (where work is written): `/site/sandbox` (`highlight: lab`). How the nest works: `/site/sandbox-how-the-lab-works`. Git branch/worktree: `POST /api/sandbox/fork`. Trash: `GET/POST /api/sandbox/trash`.
4. Leave the encyclopedia via `POST /api/sandbox/promote` `{ "phrase": "promote to main" }` only. Do not silently copy to `data/md/`.
5. House-style rewrites of standing pages are allowed when the program names them. **Forker / project pages → `data/md/` (boot).** **Framework explain → `data/docs/` (doc).** This CONTROL file stays `content/CONTROL.md`. The **turn’s new work product still also lands in the nest.** Do not mix bins.

## Two wiki bins (fork)

This repo is a **framework**. Forkers get two decoupled sites:

| Bin | Disk | URL | Role |
| --- | --- | --- | --- |
| boot | `data/md/` | `/site/...` | User wiki. Empty-ish starter + Lab bench. |
| doc | `data/docs/` | `/docs/...` | Framework manual. Optional. |

`GET /api/pages?bin=boot|doc|all`. Pages carry `bin` and `href`. `GET /api/overview` is boot-only. `GET /api/bins` says whether `data/docs` exists. `POST /api/docs/remove` `{ "confirm": true, "phrase": "Are you sure" }` deletes that folder. Papers and boards stay. If the doc folder is missing, boot still runs and this Control page is not served as `/docs/control`. If the folder is present, the doc site pops up.
...
**One pad + one API.** If `:5175` is already listening, do not start another `npm run dev:backend` / `tsx watch main.ts` — the extra tree sits in RAM even when bind fails. The backend now exits on `EADDRINUSE`. Do not steal `:5173`. Do not rebuild Chroma or fetch PDFs every park. API `runRag` sets `RAG_BM25_ONLY=1` unless `RAG_DENSE=1`. Isolated Task returns are condensed packets (`GET/POST /api/condensed`). v1 snapshot: `/site/future`. RAM lab 2026-09-06: killed one stale `tsx watch` tree that was **not** the `:5175` listener; closed leftover Cursor renderer tabs (Cursor working set ~39 GB → ~22 GB).
...
1. **Read the board** — `inbox/PENDING`, `inbox/message.md`, `inbox/latest.png`, `inbox/meta.json`. If `meta.json` has `boardId` / `pageId` / `paperIds` / `sourceType`, this is a **tied board**. `surface=clean` is an empty whiteboard: use the ids for context (the picture is not on the pad). `surface=stamped` means `latest.png` is ink **on** the page/paper/diagram export. Do not ask for a retyped prompt. **Autosave is not a site write.** Only a Park (`PENDING`) is an agent turn. **Flagged labs:** `GET /api/sandbox/flags` at turn start or when the operator says “flagged” — interesting come-back, not will-implement.
...
3. **Write Markdown** — **required Lab bench write.** Drafts go to `data/md/sandbox/` (`sandbox: true`, `parent: page:sandbox` or a lane parent, optional `sandboxLane: idea|code|research`, `depth: long`). **Chat only points at `/site/<slug>` — no chat-only encyclopedia dumps.** Honor `depth` / `pageBudget`. **Do not stop at one-liners.** New or updated architecture pages (once promoted) default to `depth: long`. Boot Overview and To-implement stay as-is (map / queue). `long` bodies include What / Why / How / Tools / MD paths / one worked example, and expand from `GET /api/meta/:id` (`summary.long`, `related`, `citations`, `parent`, `nest`, `graphDegree`) plus `GET /api/graph/neighborhood` — cited, not slogans. Paper Plain page-gist and section-gist stay collapsed chrome; the open article is the long form [@paper-plain-august-2023]. Explorations nest under `/site/sandbox` (Lab bench). How-to: `/docs/sandbox-how-the-lab-works`. Promote copies to **boot** `data/md/`. Framework explain edits go to `data/docs/`. Overview and the left TOC rebuild from `GET /api/pages?bin=boot` (lab children appear **only** under the Lab nest, not as encyclopedia peers).
...
**Three inference-time walls** (thin isolated skills **on**; imported crews **off**): **Coding** (`.cursor/skills/coding-agent/SKILL.md`) vs **Research-Generate** (`.cursor/skills/research-generate/SKILL.md`) vs **Research-Fetch** (`.cursor/skills/research-fetch/SKILL.md`). If the user says “do the research,” **clarify generate vs fetch** before acting. Fetch backbone = Paper Lantern as a **wiki verb** + **cite-or-fetch** + `GET /api/graph/neighborhood`. Coding PED = A3 turn-control (one focused change-set; do not replay expired dumps; no LLM history rewriter). A4 = lazy **hand-authored** skills in this repo only. **Sandbox fork:** `POST /api/sandbox/fork` — work in the lab until send-back; `POST /api/sandbox/propose` is human-check only (no auto-merge); no Docker Desktop. **MGM hire:** `POST /api/tripwires/hire`; `armed` false until the **user says hire MGM**; if stuck, **ask the operator** first; never production. **TraceCoder is abandoned** (we looked, we dropped it). Prefer **one mash per wall**. **To-implement** (`/site/implement`) is the Queue highlight. Lab writes: `/site/sandbox`. See `/site/agents` and `/site/knowledge-graph`. **Trash bin:** agents may `POST /api/sandbox/trash` **only when the operator explicitly asks**. Never auto-trash. Never encyclopedia.
...
- Nested wiki IA: child `parent:` in frontmatter → `GET /api/pages?bin=boot` `{ pages, tree }`. Left TOC expands; `/site/overview` is the **boot** map (starter + tutorial + empty summary + Lab). Hidden pages stay out. **Lab bench** sits immediately under Overview (`highlight: lab`, olive Lab badge, `/site/sandbox`). Explorations are children of `page:sandbox`. How-to: `/docs/sandbox-how-the-lab-works` (doc bin, not a lane). Framework pages: `/docs/docs-home`.
- Page length knob: frontmatter `depth` + optional `pageBudget`. House style: `/docs/wiki-writing`. New architecture pages default **long**. Overview / Implement / Sandbox highlights stay map / queue / lab.
- **To-implement** queue (`/site/implement`, `GET/POST /api/implement`). Git lab (`POST /api/sandbox/fork`). Lab Trash bin (`GET/POST /api/sandbox/trash`, 14-day forget, sandbox only). MGM hire hook (`POST /api/tripwires/hire`). TraceCoder abandoned. ClawVM = explore-only. Never “production.” Pointer only — do not adopt their runtime. Workbench nest is **removed**.
- Wiki knowledge graph: `GET /api/graph`, `GET /api/graph/neighborhood`, `/docs/knowledge-graph`, jump-to-node on pages. KG view clusters are visual, not new facts.
...
- Git lab (`GET /api/sandbox`, `POST /api/sandbox/fork`, `POST /api/sandbox/propose`). Website **Lab bench** at `/site/sandbox`. How-to: `/docs/sandbox-how-the-lab-works`. Olive block on articles is a pointer. `POST /api/sandbox/promote` copies MD after “promote to main” into **boot**. `requireDocker: false`. No auto-merge. Git lab merge still discuss.
- Internet → local papers: `/docs/papers-ingest` (`POST /api/papers/fetch`, OA only, cite-or-fetch). Boot tutorial: `/site/tutorial`.
...
`/docs/wiki-memory`: wiki is Living Papers MD + meta bus, grown collaboratively on the focused pages (no article decay, no compiler). Locked grounding is collaborative-basis. Boot pages (`data/md`) and papers do not decay.
...
1. **Required:** create `data/md/sandbox/<slug>.md` with `sandbox: true`, `sandboxLane: idea|code|research` (omit lane on the how-to child), `parent: page:sandbox` or `page:sandbox-<lane>`, optional `sandboxFor: page:<host>`, plus the usual YAML (`title`, `slug`, `id`, `tags`, `related`, `summaryShort`, `summaryLong`, `depth` default **long**, optional `pageBudget`, `nav`, `order`, `gist`, `questions`, `glossary`, `citations`). **Chat only points at `/site/<slug>`.** Do not dump the article in chat. Standing **boot** pages live under `data/md/`. Framework explain pages live under `data/docs/`. This CONTROL file stays in `content/`. `gist` / `summaryShort` are rail and meta — they are not the article. Expand `summaryLong`, `related`, citations, and graph neighbors into **cited** body sections. Do not invent.
2. Lab pages show under the Lab bench (`GET /api/pages?bin=boot` → nest children of `page:sandbox`, `GET /api/sandbox`). They stay out of Overview peers. Promote with `POST /api/sandbox/promote` `{ "slug", "phrase": "promote to main", "destSlug"? }` — copies to `data/md/` (boot). Does not merge git.
...
| GET | `/api/meta` | Metadata bus index (`?q=&type=&tag=&citation=&related=&nest=&parent=&flagged=`; `type=code` is `@chunk`) |
| GET | `/api/meta/:id` | One record — id, href, parent, nest, flags, graphDegree, citations |
| GET | `/api/graph` | One wiki knowledge graph (`?focus=&hops=&project=`). No invented edges. `project=` keeps explicit framework↔project edges |
...
| GET | `/api/sandbox/flags` | Flagged explorations (interesting, come back). Not implement-queue |
| POST | `/api/sandbox/flag` | `{ slug\|id, flagged, note? }` flag / unflag a sandbox page |
| GET | `/api/sandbox/trash` | Lab Trash bin (`?forgotten=1`). Not article decay |
| POST | `/api/sandbox/trash` | `{ slug }` dump a **sandbox** page. Agents only when the operator asks. Encyclopedia refused |
| POST | `/api/sandbox/trash/restore` | `{ slug }` restore before 14-day forget |
| GET | `/api/condensed` | Last isolated-subagent packets (ids + gist) |
| POST | `/api/condensed` | `{ gist, ids[], role?, hrefs?, citations? }` record one packet |
...
| GET | `/api/pages` | Page list plus nested IA `tree`. `?bin=boot\|doc\|all`. `?project=` is project-owned pages only |
| GET | `/api/bins` | Whether `data/docs` exists. Doc href when present |
| POST | `/api/docs/remove` | Delete `data/docs` after confirm + Are you sure. Boot untouched |
